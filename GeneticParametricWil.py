...
class Listrik:
    def __init__(self, tabelPembangkitListrik, periode, minListrik, initListrik, maxMT):
...
# Function to load parameters from a text file
...
# test1 = Listrik(tablePembangkitListrik, 6, 110, 150, 2) #UTAMA
...
test2 = Listrik(tablePembangkitListrik2, 12, 90, 150, 3) #EXPERIMENTAL
...
array = [[0 for j in range(test_from_file.getPeriode())] for i in range(test_from_file.getMaxMT())]
array = generateChromosome(test_from_file)
...
bestHistory = []
while True:
    schedule, fitness, bestChromosomeFitnessHistory = generateMaintenanceSchedule(test_from_file)
    print(schedule)
    runCount += 1
