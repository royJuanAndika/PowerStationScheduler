...
def load_parameters_from_file(filename):
    with open(filename, 'r') as file:
        ...
        min_listrik = int(lines[2].strip())
        init_listrik = int(lines[3].strip())
        max_mt = int(lines[4].strip())
...
class Listrik:
    ...
    def __init__(self, tabelPembangkitListrik, periode, minListrik, maxMT, SelectionMethod):
        ...
        self.initListrik = sum([row[0] for row in self.TabelPembangkitListrik])
...
def generateMaintenanceSchedule(listrik_instance):
    ...
    while True:
        ...
        print(listrik_instance.getSelectionMethod())
        if(listrik_instance.getSelectionMethod() == 1):
            currentPopulation = fillChild(
                population, currentPopulation, fitnessScorePopulation, mutationRate, listrik_instance
            )
        elif(listrik_instance.getSelectionMethod() == 2):
        ...
        print(bestChromosome, bestChromosomeFitness)
...
def showTable(df, runCountValue):
    ...
    runCount.innerHTML = "Run Count: " + str(runCountValue)
    
            


tablePembangkitListrikDummy = [
    [0,0],
    [10, 1],
    [20, 2],
    [15, 2],
...
# test1 = Listrik(tablePembangkitListrik, 6, 110, 2, 2) #UTAMA
...
#     [25, 4],
# ]
# test2 = Listrik(tablePembangkitListrik2, 12, 30, 3, 2) #EXPERIMENTAL

tablePembangkitListrik = []
test1 = Listrik(tablePembangkitListrikDummy, 6, 110, 2, 1) #UTAMA




array = [[0 for j in range(test1.getPeriode())] for i in range(test1.getMaxMT())]

bestHistory = []
# listrikInstance = Listrik()
tablePembangkitListrik = []

runCount = 0
    
def run(listrikInstancee):
    runCount = 0
    test1 = listrikInstancee
    tablePembangkitListrik = test1.getTabelPembangkitListrik()
    array = generateChromosome(test1)
    # print(listrikInstance.getSelectionMethod())
    # print(tablePembangkitListrik)
    while True:
